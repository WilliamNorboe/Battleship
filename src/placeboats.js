...
function placeBoats(human, ai){
    ...
    for(let i = 0; i < human.gameboard.board.length; ++i){
        ...
        for(let j = 0; j < human.gameboard.board[i].length; ++j){
            ...
            square.addEventListener("mouseover", ()=>(hoverBoat(square.id,ori,"green")));
            square.addEventListener("mouseout", ()=>(hoverBoat(square.id,ori,"white")));
            square.addEventListener("click", ()=>(placeBoat(square.id,ori,human,ai)));
